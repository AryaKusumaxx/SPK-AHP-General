...
function withPrevious(initValue, _a) {
    ...
    var values = store.writable(__spreadArray([initValue], rest, true));
    ...
    var current = {
        subscribe: store.derived(values, function ($values) { return $values[0]; }).subscribe,
    ...
    var others = __spreadArray([], Array(numToTrack), true).map(function (_, i) {
        return store.derived(values, function ($values) { return $values[i + 1]; });
    });
